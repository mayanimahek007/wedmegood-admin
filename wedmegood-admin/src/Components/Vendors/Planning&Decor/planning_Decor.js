...
const PlanningDecor = () => {
  ...
  const addPhoto = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:3000/api/v1/planningDecor/create", formData, {
  ...
  const deletePhotographer = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.delete(`http://localhost:3000/api/v1/planningDecor/delete/${id}`);
  ...
  const updateVenueData = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`http://localhost:3000/api/v1/planningDecor/update/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
                    ...
                    <tbody>
                      {photographers.length > 0 ? (
                        photographers.map((photographer, index) => (
                          <tr key={index}>
                            <td>{photographer.planningName}</td>
                            <td>{new Date(photographer.planningDate).toLocaleDateString()}</td>
                            <td>{photographer.planningEmail}</td>
                            <td>{photographer.planningNumber}</td>
                            <td>${photographer.planningPrice}</td>
                            <td>{photographer.planningPriceInfo}</td>
                            <td>{photographer.planningDetails}</td>
