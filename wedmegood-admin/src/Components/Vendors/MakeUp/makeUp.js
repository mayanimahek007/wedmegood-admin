...
const MakeUp = () => {
  ...
  const addPhoto = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:3000/api/v1/makeup/create", formData, {
  ...
  const deletePhotographer = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.delete(`http://localhost:3000/api/v1/makeup/delete/${id}`);
  ...
  const updateVenueData = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`http://localhost:3000/api/v1/makeup/update/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
